# Yoshi Go API Library

<!-- x-release-please-start-version -->

<a href="https://pkg.go.dev/github.com/yoshi-ai-dev/yoshi-go"><img src="https://pkg.go.dev/badge/github.com/yoshi-ai-dev/yoshi-go.svg" alt="Go Reference"></a>

<!-- x-release-please-end -->

The Yoshi Go library provides convenient access to the [Yoshi REST API](https://docs.yoshi.ai)
...
```go
import (
	"github.com/yoshi-ai-dev/yoshi-go" // imported as yoshi
)
```
...
```sh
go get -u 'github.com/yoshi-ai-dev/yoshi-go@v0.2.0'
```
...
The yoshi library uses the [`omitzero`](https://tip.golang.org/doc/go1.24#encodingjsonpkgencodingjson)
semantics from the Go 1.24+ `encoding/json` release for request fields.

Required primitive fields (`int64`, `string`, etc.) feature the tag <code>\`api:"required"\`</code>. These
fields are always serialized, even their zero values.

Optional primitive types are wrapped in a `param.Opt[T]`. These fields can be set with the provided constructors, `yoshi.String(string)`, `yoshi.Int(int64)`, etc.
...
```go
p := yoshi.ExampleParams{
	ID:   "id_xxx",            // required property
	Name: yoshi.String("..."), // optional property

	Point: yoshi.Point{
		X: 0,            // required field will serialize as 0
		Y: yoshi.Int(1), // optional field will serialize as 1
		// ... omitted non-required fields will not be serialized
	},

	Origin: yoshi.Origin{}, // the zero value of [Origin] is considered omitted
}
```
...
custom := param.Override[yoshi.FooParams](12)
```
...
See the [full list of request options](https://pkg.go.dev/github.com/yoshi-ai-dev/yoshi-go/option).
...
for iter.Next() {
	transactionListResponse := iter.Current()
...
`*yoshi.Error`. This contains the `StatusCode`, `*http.Request`, and
...
We also provide a helper `yoshi.File(reader io.Reader, filename string, contentType string)`
...
We are keen for your feedback; please open an [issue](https://www.github.com/yoshi-ai-dev/yoshi-go/issues) with questions, bugs, or suggestions.
